...
data class AssetRef(
    ...
    companion object {
        ...
        fun fromString(str: String): Result<AssetRef> {
            ...
            if (items.size != 2) {
                return Result.failure(Error("Unexpected length of given asset ref. Make sure that the asset ref string contains exactly 2 items separated by '$DELIMITER'!"))
            }
            val retSource = AssetSource.fromString(items[0]).getOrElse {
                return Result.failure(Error(it))
            }
